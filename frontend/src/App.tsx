...
function App() {
	...
	const [selectedAgent, setSelectedAgent] = useState<"gemini" | "claude">("gemini");
	...
	const handleSendMessage = async (content: string) => {
		...
		try {
			...
			// Send the message to the API with agent
			const response = await sendMessage(updatedMessages, selectedAgent);
	...
	const handleRegenerateResponse = async () => {
		...
		try {
			...
			const response = await sendMessage(messagesToKeep, selectedAgent);
		...
		<>
			...
			<AppContainer>
				...
				<MainContent>
					{activeConversation ? (
						<>
							<Chat messages={messages} loading={loading} onRegenerate={handleRegenerateResponse} />
							<ChatInput onSendMessage={handleSendMessage} isLoading={loading} />
							<div style={{ padding: "1rem", textAlign: "right" }}>
								<label htmlFor="agent-select" style={{ marginRight: 8 }}>
									AI Agent:
								</label>
								<select
									id="agent-select"
									value={selectedAgent}
									onChange={(e) => setSelectedAgent(e.target.value as "gemini" | "claude")}
